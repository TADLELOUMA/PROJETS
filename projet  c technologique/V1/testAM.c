...
/*test is_game_possible*/
bool test_is_game_possible(){
  game jeu = new_game_ext(NB_COL,NB_LINE);
  add_mirror_ext(jeu, ANTIMIRROR, 0, 3);
  add_mirror_ext(jeu, MIRROR, 3, 3);
  add_mirror_ext(jeu,ANTIMIRROR, 0, 2);
  ...
  if(is_game_possible(jeu) == false){
    fprintf(stderr,"jeu pas possible");
    rep = false;
  }
  return rep;
}
...
/*tester le sol_recursive*/
bool test_sol_recursive(){
  game jeu = new_game_ext(NB_COL,NB_LINE);
  add_mirror_ext(jeu, ANTIMIRROR, 0, 3);
  add_mirror_ext(jeu, MIRROR, 3, 3);
  add_mirror_ext(jeu,ANTIMIRROR, 0, 2);
  ...
  if(sol_recursive(jeu, 0, false) == false){
    fprintf(stderr,"solution no found");
    rep = false;
  }
  return rep;
}
...
int main(void){
  ...
  result = test_sol_recursive() && result;
  result = test_is_game_possible() && result;
  if(result){
