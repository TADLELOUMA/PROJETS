...
void render(SDL_Window* win, SDL_Renderer* ren, Env * env)
{
  ...
  /* define constant  */
  int y1 = 6*h/(2*height+10);
  int y2 = (2*height+6)*h/(2*height+10);
  ...
  for (int x = 0; x <= width ; x++){
    SDL_RenderDrawLine(ren, (2*x+2)*w/(2*width+4), 6*h/(2*height+10), (2*x+2)*w/(2*width+4),(2*height+6)*h/(2*height+10));
  }
  for(int y =0;y<=height;y++){
    SDL_RenderDrawLine(ren,2*w/(2*width+4), (2*y+6)*h/(2*height+10),(2*width+2)*w/(2*width+4),(2*y+6)*h/(2*height+10));
  }
